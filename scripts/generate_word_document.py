...
def setup_document_styles(doc):
    ...
    # Настройка стиля Heading 2 (для разделов)
    ...
    # Настройка стиля Heading 3 (для подразделов)
    ...
    style_heading3.font.italic = True
...
def add_bibliography(doc):
    ...
    # Здесь можно добавить список литературы из отдельного файла, если он существует
    # Или добавить стандартный шаблон списка литературы
    
    p = doc.add_paragraph('1. Иванов И.И. Название книги. - М.: Издательство, 2023. - 123 с.', style='Normal')
    p.paragraph_format.first_line_indent = Cm(-0.63)
    p.paragraph_format.left_indent = Cm(1.25)
    
    p = doc.add_paragraph('2. Петров П.П. Название статьи // Название журнала. - 2022. - №5. - С. 10-15.', style='Normal')
    p.paragraph_format.first_line_indent = Cm(-0.63)
    p.paragraph_format.left_indent = Cm(1.25)
    
    p = doc.add_paragraph('3. Сидоров С.С. Название диссертации: дис. ... канд. наук. - СПб., 2021. - 150 с.', style='Normal')
    p.paragraph_format.first_line_indent = Cm(-0.63)
    p.paragraph_format.left_indent = Cm(1.25)
